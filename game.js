if (!window.__arenaDuelFpsLoaded) {
window.__arenaDuelFpsLoaded = true;
...
function generateProps(size) {
  ...
  for (let i = 0; i < propCount; i++) {
    ...
    state.props.push({
      ...
      seed: i * 37 + Math.floor(rand(0, 999))
    });
  }
}
...
function shoot() {
  ...
  ejectCasing();
  const recoilPower = profile.type === "heavy" ? .58 : profile.type === "shotgun" || profile.type === "marksman" ? .46 : profile.type === "pistol" ? .28 : .36;
  state.recoil = Math.max(state.recoil, recoilPower);
  state.muzzleFlash = profile.type === "energy" ? .11 : .09;
  state.shake = Math.max(state.shake, profile.type === "heavy" ? .22 : profile.type === "shotgun" ? .16 : .12);
  if (hit) {
...
function drawGroundDetail(width, height, horizon, theme) {
  ...
  if (hardSurface) {
    ...
    ctx.strokeStyle = theme.scene === "mall" ? "rgba(38, 42, 43, .18)" : "rgba(8, 10, 12, .2)";
    ctx.lineCap = "round";
    for (let i = 0; i < 72; i++) {
      const amount = (i + 1) / 72;
      const y = horizon + Math.pow(amount, 1.4) * (height - horizon);
      const x = wrapScreen(hashNoise(i, 131) * width - backgroundPan(width, .22) * amount, width);
      ctx.globalAlpha = .12 + amount * .32;
      ctx.lineWidth = 1 + amount * 3;
      ctx.beginPath();
      ctx.moveTo(x, y);
      ctx.lineTo(x + (hashNoise(i, 137) - .5) * 120 * amount, y + 8 * amount);
      ctx.stroke();
    }
  } else {
  ...
  ctx.globalAlpha = 1;
  ctx.fillStyle = hardSurface ? "rgba(255,255,255,.035)" : "rgba(18, 13, 8, .16)";
  for (let i = 0; i < 120; i++) {
    const amount = (i + 1) / 120;
    const y = horizon + Math.pow(amount, 1.55) * (height - horizon);
    const x = wrapScreen(hashNoise(i, 411) * width + backgroundPan(width, .08) * amount, width);
    const s = .6 + amount * 2.8;
    ctx.globalAlpha = .08 + amount * .32;
    ctx.fillRect(x, y, s, s * .35);
  }
  ctx.restore();
}
...
function drawEnvironmentalVignette(width, height) {
  ...
  const theme = backgroundThemes[state.map.name] || backgroundThemes.Forest;
  const tint = {
    desert: "rgba(255, 194, 112, .08)",
    forest: "rgba(122, 166, 111, .06)",
    mall: "rgba(255, 238, 210, .05)",
    space: "rgba(96, 180, 255, .07)",
    city: "rgba(190, 205, 215, .05)",
    warehouse: "rgba(255, 206, 142, .05)",
    office: "rgba(175, 210, 230, .05)",
    village: "rgba(210, 155, 104, .07)"
  }[theme.scene] || "rgba(255, 255, 255, .04)";
  ctx.fillStyle = tint;
  ctx.fillRect(0, 0, width, height);
...
function drawBuildingExterior(x, y, scale, building, screenWidth, screenHeight) {
  ...
  const type = building.structureType || "office";
  ctx.save();
  ctx.fillStyle = "rgba(4, 7, 9, .5)";
  ctx.beginPath();
  ctx.ellipse(x, y + 5, w * .68, Math.max(12, h * .1), 0, 0, Math.PI * 2);
  ctx.fill();
...
function drawProp(x, y, scale, prop) {
  const w = clamp(prop.radius * scale, 12, 92);
  const h = clamp(prop.height * scale, 18, 132);
  ctx.save();
  ctx.fillStyle = "rgba(0,0,0,.34)";
  ctx.beginPath();
  ctx.ellipse(x, y + h * .04, w * .72, Math.max(5, h * .1), 0, 0, Math.PI * 2);
  ctx.fill();

  const dark = mixHexColors(prop.tint || state.map.wall, "#05080a", .55);
  const mid = mixHexColors(prop.tint || state.map.prop, "#111820", .22);
  const light = mixHexColors(prop.tint || state.map.prop, "#f3ead0", .18);
  ctx.strokeStyle = "rgba(6, 8, 10, .72)";
  ctx.lineWidth = Math.max(1, scale * 1.4);

  if (["crate", "cover", "cargo", "supply", "sandbags", "ruin", "cart", "desk"].includes(prop.type)) {
    const stackCount = prop.type === "sandbags" ? 3 : prop.type === "ruin" ? 2 : 1;
    for (let row = 0; row < stackCount; row++) {
      const rowY = y - h * (.2 + row * .24);
      const rowW = w * (1.55 - row * .18);
      const rowH = h * (prop.type === "sandbags" ? .22 : .48);
      const grad = ctx.createLinearGradient(x - rowW / 2, rowY - rowH, x + rowW / 2, rowY);
      grad.addColorStop(0, light);
      grad.addColorStop(.52, mid);
      grad.addColorStop(1, dark);
      ctx.fillStyle = grad;
      ctx.beginPath();
      if (prop.type === "sandbags") {
        ctx.roundRect(x - rowW / 2, rowY - rowH, rowW, rowH, rowH * .4);
      } else {
        ctx.rect(x - rowW / 2, rowY - rowH, rowW, rowH);
      }
      ctx.fill();
      ctx.stroke();
      ctx.strokeStyle = "rgba(255,255,255,.13)";
      ctx.beginPath();
      ctx.moveTo(x - rowW * .38, rowY - rowH * .72);
      ctx.lineTo(x + rowW * .36, rowY - rowH * .18);
      ctx.stroke();
      ctx.strokeStyle = "rgba(6, 8, 10, .72)";
    }
  } else if (["barrel", "fountain", "well", "plant", "antenna"].includes(prop.type)) {
    const grad = ctx.createLinearGradient(x - w, y - h, x + w, y);
    grad.addColorStop(0, light);
    grad.addColorStop(.5, mid);
    grad.addColorStop(1, dark);
    ctx.fillStyle = grad;
    ctx.beginPath();
    ctx.ellipse(x, y - h * .5, w * .52, h * .48, 0, 0, Math.PI * 2);
    ctx.fill();
    ctx.stroke();
    if (prop.type === "plant") {
      ctx.fillStyle = "#3d7952";
      for (let leaf = 0; leaf < 6; leaf++) {
        const a = leaf / 6 * Math.PI * 2;
        ctx.beginPath();
        ctx.ellipse(x + Math.cos(a) * w * .22, y - h * .92 + Math.sin(a) * h * .09, w * .28, h * .1, a, 0, Math.PI * 2);
        ctx.fill();
      }
    }
    if (prop.type === "antenna") {
      ctx.strokeStyle = light;
      ctx.beginPath();
      ctx.moveTo(x, y - h * .98);
      ctx.lineTo(x, y - h * 1.55);
      ctx.moveTo(x - w * .5, y - h * 1.18);
      ctx.lineTo(x + w * .5, y - h * 1.18);
      ctx.stroke();
    }
  } else if (["forklift", "jeep", "cart"].includes(prop.type)) {
    ctx.fillStyle = dark;
    ctx.fillRect(x - w * .9, y - h * .42, w * 1.8, h * .34);
    ctx.fillStyle = mid;
    ctx.fillRect(x - w * .45, y - h * .78, w * .8, h * .36);
    ctx.fillStyle = "rgba(8, 10, 12, .9)";
    for (const side of [-.55, .55]) {
      ctx.beginPath();
      ctx.arc(x + w * side, y - h * .08, Math.max(4, w * .18), 0, Math.PI * 2);
      ctx.fill();
    }
    ctx.strokeRect(x - w * .9, y - h * .42, w * 1.8, h * .34);
  } else if (["shop", "bench", "pillar", "tower", "server", "console", "pod"].includes(prop.type)) {
    const propHeight = ["shop", "bench"].includes(prop.type) ? h * .45 : prop.type === "pillar" ? h * .82 : h;
    const propWidth = prop.type === "bench" ? w * 1.7 : prop.type === "shop" ? w * 1.4 : w * 1.1;
    const grad = ctx.createLinearGradient(x - w / 2, y - h, x + w / 2, y);
    grad.addColorStop(0, light);
    grad.addColorStop(.5, mid);
    grad.addColorStop(1, dark);
    ctx.fillStyle = grad;
    ctx.fillRect(x - propWidth * .5, y - propHeight, propWidth, propHeight);
    ctx.strokeRect(x - propWidth * .5, y - propHeight, propWidth, propHeight);
    ctx.fillStyle = "rgba(255,255,255,.12)";
    ctx.fillRect(x - propWidth * .35, y - propHeight * .85, propWidth * .22, propHeight * .62);
    if (prop.type === "server" || prop.type === "console") {
      ctx.fillStyle = "#74d7ff";
      for (let led = 0; led < 4; led++) ctx.fillRect(x + w * .2, y - h * (.78 - led * .14), Math.max(2, w * .08), Math.max(2, h * .03));
    }
  } else {
    ctx.fillStyle = mid;
    ctx.fillRect(x - w / 2, y - h, w, h);
...
function characterScreenHeight(scale) {
  const maxHeight = clamp(canvas.clientHeight * .58, 150, 430);
...
function drawEnemy(x, y, scale, enemy) {
  ...
  vest.addColorStop(0, "#252a2d");
  vest.addColorStop(.45, armorColor);
  vest.addColorStop(1, "#2d3337");
  ctx.fillStyle = vest;
  ctx.beginPath();
  ...
  ctx.strokeStyle = mixHexColors(armorColor, "#171c20", .38);
  ctx.lineWidth = Math.max(3, w * .2);
  ctx.beginPath();
